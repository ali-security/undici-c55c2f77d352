...
test('forwards a custom connect function to upstream pools', (t) => {
  const connect = (opts, callback) => {
    callback(new Error('custom connector'))
  }
  let capturedOptions
  const pool = new BalancedPool([], {
    connect,
  ...
  assert.strictEqual(capturedOptions.connect, connect)
})

test('forwards tls options, including non-serializable ones, to upstream pools', (t) => {
  const ca = fs.readFileSync(path.join(__dirname, '..', 'fixtures', 'ca.pem'))
  const checkServerIdentity = () => new Error('rejected by custom checkServerIdentity')
  let capturedOptions
  const pool = new BalancedPool([], {
    tls: { servername: 'agent1', ca, checkServerIdentity },
    factory (origin, options) {
      capturedOptions = options
      return new Pool(origin, options)
    }
  ...
  assert.strictEqual(capturedOptions.tls.servername, 'agent1')
  assert.strictEqual(capturedOptions.tls.checkServerIdentity, checkServerIdentity)
  assert.ok(Buffer.isBuffer(capturedOptions.tls.ca))
  assert.ok(capturedOptions.tls.ca.equals(ca))
})

test('forwards TLS certificate validation callbacks given via the tls option', async (t) => {
  const server = https.createServer({
    key: fs.readFileSync(path.join(__dirname, '..', 'fixtures', 'key.pem')),
    cert: fs.readFileSync(path.join(__dirname, '..', 'fixtures', 'cert.pem'))
  }, (req, res) => {
