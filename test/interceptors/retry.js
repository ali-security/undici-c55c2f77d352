...
test('should reject the original response body when a retry returns a non-retryable status and throwOnError is false', async t => {
  t = tspl(t, { plan: 3 })
  ...
  const client = new Client(
    `http://localhost:${server.address().port}`,
    { bodyTimeout: 100 }
  ).compose(retry({ minTimeout: 10, throwOnError: false }))
